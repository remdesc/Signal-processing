...
def record_signal(N=3,fs=22050, play = False, lowpass=False):
    """To record N individual patterns within a signal x
    at default sampling frequency fs=22050 Hz """

    print("Appuyer sur une touche pour lancer l'enregistrement.\n")
    input()
    print('Enregistrement en cours : durée '+str(N) +' secondes...\n')
    sd.default.samplerate = fs
    duration = N  # seconds
    x = sd.rec(int(duration * fs), samplerate=fs, channels=1)

    sd.wait()  # gives hands back if finished

    print('Enregistrement terminé.\n')
    # Normalisation du signal
    x = concatenate(x-x.mean())/max(abs(x-mean(x)))
    # passe-bas (necessaire pour une bonne detection)
    # LOW PASS
    if lowpass is True:
        b, a = sg.butter(10, 0.4) # butterworth order 10, f0 = 0.1
        x = sg.lfilter(b, a, x)
        # Normalisation du signal
        x = (x-x.mean())/max(abs(x-mean(x)))
    if play==True:
        sd.play(x, fs)
